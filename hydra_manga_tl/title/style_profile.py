...
class StrokeProfile:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any] | None) -> "StrokeProfile":
        payload = payload or {}
        width = payload.get("width")
...
class TypographyProfile:
    ...
    def from_dict(cls, payload: dict[str, Any] | None) -> "TypographyProfile":
        ...
        return cls(
            ...
            line_spacing=float(payload["line_spacing"]) if payload.get("line_spacing") is not None else None,
            classification=str(payload["classification"]) if payload.get("classification") is not None else None,
...
class TitleStyleProfile:
    ...
    def from_dict(cls, payload: dict[str, Any] | None) -> "TitleStyleProfile":
        ...
        return cls(
            version=int(safe.get("version", 1 if payload else 2) or 2),
            ...
            stroke=StrokeProfile.from_dict(safe.get("stroke")) if safe.get("stroke") is not None else None,
            ...
            alignment=str(safe["alignment"]) if safe.get("alignment") is not None else None,
            spacing=float(safe["spacing"]) if safe.get("spacing") is not None else None,
            blend_mode=str(safe["blend_mode"]) if safe.get("blend_mode") is not None else None,
            confidence={str(key): float(value) for key, value in dict(safe.get("confidence", {})).items()},
