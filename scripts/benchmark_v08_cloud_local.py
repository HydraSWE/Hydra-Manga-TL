...
def _shared_ocr(items: list[dict[str, str]], root: Path) -> dict[str, Any]:
    from hydra_manga_tl.ocr_service import OCRService
    from hydra_manga_tl.preprocessor import prepare_ocr_image
...
def _run_engine(
    ...
    from hydra_manga_tl.paths import PATHS
    from hydra_manga_tl.pipeline import PipelineWorker
    from hydra_manga_tl.settings import SETTINGS
...
def main() -> int:
    ...
    root = (
        args.output
        or Path("outputs") / f"v08_cloud_local_cool3_{args.start}_{args.end}_{stamp}"
    ).resolve()
